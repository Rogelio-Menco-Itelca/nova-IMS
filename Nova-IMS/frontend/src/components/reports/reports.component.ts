...
export class ReportsComponent implements OnInit {
  ...
  filters = this.fb.group({
    from: [''], to: [''], status: [''], type: [''], priority: [''], operator: [''],
  });
  ...
  load(): void {
    ...
    this.svc.getSummary({
      ...
      status: f.status || undefined, type: f.type || undefined,
      priority: f.priority || undefined, operator: f.operator || undefined,
    }).subscribe({
      next:  (d) => { this.data.set(d); this.isLoading.set(false); },
      error: (e) => { this.error.set(e?.error?.error?.message || 'Error al cargar reportes'); this.isLoading.set(false); },
    });
  }

  resetFilters(): void { this.filters.reset(); this.searchText.set(''); this.load(); }
  onSearchText(e: Event) { this.searchText.set((e.target as HTMLInputElement).value); }

  barWidth(value: number, max: number): number { return max > 0 ? Math.round((value / max) * 100) : 0; }
  maxOf(arr: { value: number }[]): number { return arr.length ? Math.max(...arr.map(x => x.value)) : 1; }
  maxDailyTotal(daily: ReportSummary['daily']): number { return daily.length ? Math.max(...daily.map(d => d.total)) : 1; }
  ...
  priorityColor(p: string): string {
    return { 'Crítica':'text-red-400','Alta':'text-orange-400','Media':'text-yellow-400','Baja':'text-green-400' }[p] ?? 'text-gray-400';
  }
  priorityBar(p: string): string {
    return { 'Crítica':'bg-red-500','Alta':'bg-orange-500','Media':'bg-yellow-500','Baja':'bg-green-500' }[p] ?? 'bg-gray-500';
  }
  ...
  exportCSV(): void {
    ...
    const headers = ['ID','Tipo','Prioridad','Estado','Origen','Teléfono','Ubicación','Operador','Fecha creación','Última actualización'];
    const lines = [
      headers.join(','),
      ...rows.map(r => [
